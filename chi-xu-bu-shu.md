...
# 总结
PSET在部署这里比较AWS功能要薄弱些，对部署类型的支持也较少。AWS部署的特性如下：
### 1. 部署配置
...
### 4. 部署类型
AWS部署类型有`蓝绿部署`和`就地部署`这个是PSET不具备的。`蓝/绿部署`使用两个完全相同的环境。客户机使用一个活动环境时，可更新另一个环境而不干扰该活动环境。然后，在另一环境就绪时，您会设置负载均衡器以将客户机流量发送至已更新环境。以此方式更新环境允许实现不间断部署，并使第二个工作环境一直保持就绪状态，以防该活动环境发生任何错误。
![deploy-type](/assets/2019-02-21_202358.png)
### 5. 部署跟踪
AWS记录了部署事件执行的动作，跟踪部署的状态，见下图。
![track-deploy](/assets/2019-02-21_200108.png)
### 6. 复制部署
AWS创建一个部署需要填写的信息不少，用户可以通过复制部署来减少用户输入。
![copy-deploy](/assets/2019-02-21_202058.png)